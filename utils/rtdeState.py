...
ROBOT_PORT = 30004
config_filename = 'rtde_tracking.xml'
...
class RtdeState:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.set_q = self.con.send_output_setup(set_q_names, set_q_types)

            set_xyz_names, set_xyz_types = conf.get_recipe('set_xyz')
            self.set_xyz = self.con.send_input_setup(set_xyz_names, set_xyz_types)
            
            set_v_xyz_names, set_v_xyz_types = conf.get_recipe('set_v_xyz')
            self.set_v_xyz = self.con.send_input_setup(set_v_xyz_names, set_v_xyz_types)

            set_v_xyz_fix_names, set_v_xyz_fix_types = conf.get_recipe('set_v_xyz_fix')
            self.set_v_xyz_fix = self.con.send_input_setup(set_v_xyz_fix_names, set_v_xyz_fix_types)
            ...
            suction_names, suction_types = conf.get_recipe('suction')
            self.suction = self.con.send_input_setup(suction_names, suction_types)
